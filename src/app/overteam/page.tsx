...
export default function OurTeam() {
  return (
    <section className="py-20 px-2 sm:px-4 bg-gradient-to-br from-[#0a0a23] to-gray-900 text-white">
      <div className="w-full text-center">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {teamMembers.map((member, idx) => (
            <div
              key={idx}
              className="relative group w-full max-w-[300px] mx-auto overflow-hidden rounded-xl shadow-xl bg-white/5 backdrop-blur-md border border-white/10"
            >
              {/* Main Image */}
              <Image
                src={member.image}
                ...
                width={400}
                height={400}
                className="w-full h-[380px] object-cover transition-all duration-500 group-hover:scale-110"
              />

              {/* Overlay on Hover */}
              <div className="absolute inset-0 bg-black/80 text-white flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-500 px-4 text-center">
                {/* Avatar Circular */}
